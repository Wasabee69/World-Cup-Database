...
echo "$(expr $($PSQL "SELECT SUM(winner_goals) FROM games") + $($PSQL "SELECT SUM(opponent_goals) FROM games"))"
...
echo "$(awk "BEGIN { printf \"%.16f\", $($PSQL "SELECT AVG(winner_goals) FROM games") + $($PSQL "SELECT AVG(opponent_goals) FROM games") }")"
